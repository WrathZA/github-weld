...
- NEVER create issues for work you are about to implement in the same session — the issue would be created and immediately closed with no meaningful history; for same-session work use `gh-weld-adopt` instead (it creates the issue, branch, and commit for work you're implementing now), or `gh-weld-ship` once the work is done on a feature branch (it links the issue to the branch and closes it on merge)
